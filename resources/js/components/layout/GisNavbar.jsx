...
export default function GisNavbar() {
    ...
    const handleSearchSubmit = async (e) => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                alert(`⚠️ Wilayah "${queryText}" tidak terdeteksi di area Bandung Barat.`);
            }
        } catch (error) {
    ...
    const handleShareCoordinates = async () => {
        const shareText = `LINTAS KBB Monitoring:\nFokus kordinat saat ini: ${mapCenter[0].toFixed(5)}, ${mapCenter[1].toFixed(5)} (Zoom: ${mapZoom})`;
    ...
    return (
        <nav className="w-full h-16 px-6 flex items-center justify-between bg-[#1E293B] border-b border-white/10 text-white select-none">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col leading-none">
                        <span className="text-sm font-black tracking-tight text-white uppercase">
                            LINTAS <span className="text-[#00e5ff]">KBB</span>
                        </span>
            ...
            <div className="hidden md:flex flex-1 max-w-lg mx-8 relative">
                <form onSubmit={handleSearchSubmit} className="w-full relative group">
                    <button type="submit" disabled={isSearching} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#00e5ff]">
                        {isSearching ? <Loader2 size={16} className="animate-spin text-[#00e5ff]" /> : <Search size={16} />}
                    </button>
                    <input
                        type="text"
                        value={searchQuery}
                        ...
                        className="w-full bg-slate-900/60 border border-white/10 py-2.5 pl-12 pr-6 text-xs text-white placeholder:text-slate-400 focus:outline-none focus:border-[#00e5ff] focus:bg-[#0f172a] transition-all rounded-none"
                    />
                </form>
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                <button
                    ...
                    disabled={isSyncing}
                    className="flex items-center gap-2 px-3 py-2 text-slate-300 hover:text-white hover:bg-white/5 transition-all rounded-none outline-none"
                    title="Sinkronisasi seluruh data spasial langsung dari PostgreSQL"
                >
                    {isSyncing ? <Loader2 size={14} className="animate-spin text-[#00e5ff]" /> : <RefreshCw size={14} />}
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 transition-all rounded-none outline-none ${isCopied ? 'text-[#00e5ff]' : 'text-slate-300 hover:text-white hover:bg-white/5'}`}
                ...
                {/* Profil Sesi (Standard Laravel Auth Representation) */}
                <div className="flex items-center gap-3 pl-2">
                    <div className="hidden sm:flex flex-col items-end leading-none">
                        <span className="text-[10px] font-black text-white uppercase tracking-tight truncate max-w-28">
                            {currentUser.name}
                        </span>
                        <span className="text-[8px] text-[#00e5ff] font-black uppercase tracking-widest mt-0.5">
                            {currentUser.role}
                        </span>
                    </div>

                    <div className="w-9 h-9 bg-slate-800 text-[#00e5ff] flex items-center justify-center text-xs font-black border-2 border-white/10 rounded-none shrink-0 shadow-inner">
                        <User size={16} />
                    </div>
                </div>
