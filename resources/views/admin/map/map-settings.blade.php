...
<style>
    ...
    #geo-search, #input-lat, #input-lng {
        ...
        <div class="bg-white border border-slate-100">
            <div class="flex items-center gap-2 py-2 px-3">
                ...
                <button id="btn-search-loc" class="bg-slate-900 text-white px-4 py-2 hover:bg-indigo-600 transition-all flex items-center justify-center gap-1.5 font-bold text-[10px] tracking-wider">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    Cari
                </button>
            </div>
        </div>
                ...
                <div class="group relative bg-white p-1 border border-slate-100 h-full overflow-hidden min-h-0">
                    ...
                    <div class="custom-layer-control">
                        <div class="layer-button">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5zM2 12l10 5 10-5M2 17l10 5 10-5"></path></svg>
                        </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        baseLayers.roadmap = L.tileLayer('https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', { maxZoom: 20 });
        baseLayers.satellite = L.tileLayer('https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', { maxZoom: 20 });

        map = L.map('map-aset', { zoomControl: false, layers: [baseLayers.light] }).setView(MAP_SETTINGS.center, MAP_SETTINGS.defaultZoom);
